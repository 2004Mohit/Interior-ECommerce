...
export const VendorReviews = () => {
  ...
  /**
   * Safely convert any review field into searchable text.
   * This prevents crashes when a database field is null/undefined.
   */
  const safeText = (value) => String(value ?? "");

  const normalizedSearch = safeText(search).trim().toLowerCase();

  const filteredReviews = reviews.filter((r) => {
    const productName = safeText(r?.productName).toLowerCase();
    const customerName = safeText(r?.customerName).toLowerCase();
    const comment = safeText(r?.comment).toLowerCase();
    const headline = safeText(r?.headline).toLowerCase();
      ...
      <div className="gm-panel p-6 sm:p-8 rounded-3xl border border-[#D9E2EA] bg-[#FEFEFE] grid grid-cols-1 lg:grid-cols-12 gap-6 items-center shadow-xs">
        ...
        <div className="lg:col-span-4 text-center lg:border-r border-[#D9E2EA] lg:pr-6 space-y-2">
          ...
          <div className="flex justify-center text-[#3C7DDA] my-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star
                key={star}
                className={`w-5 h-5 ${
                  Number(metrics?.averageRating || 0) >= star
          ...
          <p className="text-xs text-[#606460]">
            Based on{" "}
            <strong>
              {Number(metrics?.totalReviews || 0)} verified customer review(s)
            </strong>
          </p>
        </div>
        ...
        <div className="lg:col-span-8 space-y-2 text-xs">
          {[5, 4, 3, 2, 1].map((stars) => {
            const count = Number(metrics?.ratingBreakdown?.[stars] || 0);
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredReviews.map((rev) => {
            const productName = safeText(rev?.productName) || "Product";
            const productCategory =
              safeText(rev?.productCategory) || "Construction Product";
            const customerName =
              safeText(rev?.customerName) || "Verified Customer";
            const customerLocation =
              safeText(rev?.customerLocation) || "Pune & PCMC";
            const comment = safeText(rev?.comment);
            const headline = safeText(rev?.headline);
            const productSlug = safeText(rev?.productSlug);
            const reviewDate = rev?.reviewDate
              ? new Date(rev.reviewDate)
              : null;
            ...
            return (
              <div
                key={
                  rev?.id ||
                  `${productName}-${customerName}-${rev?.reviewDate || Math.random()}`
                }
              ...
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2">
                      ...
                      {Boolean(rev?.isVerifiedPurchase) && (
                ...
                <div className="pt-3 border-t border-[#D9E2EA] flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] text-[#606460]">
                  <div>
                    Customer:{" "}
                    <strong className="text-[#282926]">{customerName}</strong> (
                    {customerLocation})
                  </div>
